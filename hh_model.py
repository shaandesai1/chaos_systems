...
class HNN(nn.Module):
    ...
    def time_deriv(self, x, t):
        ...
        state_dim = int(self.indim / 2)
        return torch.cat([dF2[:,state_dim:int(2*state_dim)].reshape(-1, state_dim), -dF2[:, 0:state_dim].reshape(-1, state_dim)], 1)
