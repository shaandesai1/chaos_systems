...
def pend_chaos(num_trajectories, T_max, dt, srate, seed):
    ...
    def hamiltonian_fn(coords):
        ...
        H = 9.81 * (1 - cos(coords[:, 0])) + (coords[:, 1] ** 2) / 2 - 1 * cos(3 * coords[:, 2]) * coords[:,
                                                                                                      0]  # pendulum hamiltonian
        return H
...
def painleve_I(num_samples, T_max, dt, srate, noise_std=0, seed=3):
    """simple pendulum"""
