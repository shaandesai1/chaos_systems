...
train_data = painleve_I(num_trajectories, T_max, dt, srate, 0,1)#pendulum(num_trajectories, T_max, dt, srate, noise_std=0, seed=3)
valid_data = painleve_I(n_test_traj, T_max_t, dt, srate, 0,1)#pendulum(n_test_traj, T_max_t, dt, srate, noise_std=0, seed=5)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for batch_i, (q, q_next, energy_, qdx,tevals) in enumerate(data_dict[phase]):
                ...
                if new_reg:
                    derivs = model.time_deriv(q, tevals)
...
trained_model = train_model(model_ft, optimizer_ft,lr_sched, num_epochs=500, energy_term=False,integrator_embedded=True, reg_grad=False)
