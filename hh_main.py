...
tnow, tnext,tenergy,dtnow = nownext(train_data, num_trajectories, T_max, dt, srate)
vnow, vnext,venergy,dvnow = nownext(valid_data, n_test_traj, T_max_t, dt, srate)
traindat = pendpixdata(tnow, tnext,tenergy,dtnow)
...
def train_model(model, optimizer, scheduler,num_epochs=1,integrator_embedded=True):
    for epoch in range(num_epochs):
        ...
        for phase in ['train','valid']:
            ...
            running_loss = 0.0
            # Iterate over data.
            for batch_i, (q, q_next,energy_,dq) in enumerate(data_dict[phase]):
                if phase == 'train':
                    optimizer.zero_grad()
